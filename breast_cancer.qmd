...
```{r}
table(sce$CellType) / ncol(sce) * 100
```

question 2

mixed but tumour tries to bulk

question 3

```{r}
sum(assay(sce, "logcounts")["CD274", ] != 0) / ncol(sce) * 100
```


```{r}
plotExpression(sce, features = "CD274", colour_by = "CellType")
```
