...
generate big amount of data. The main two approaches involve long-read
sequencing with technology and short-read sequencing with an Illumina machine.
In the latter case, Polymerase Chain Reaction (PCR) is performed by the
sequence to amplify the library and quantify gene expression.
...
gene) is quantified in relation with the rest of the pool. Thus, it is essential
...
distribution and less to a poison distribution, this is due to the variability
